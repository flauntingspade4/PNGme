...
impl Png {
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        ...
        for header in Self::STANDARD_HEADER.iter() {
            to_return.push(*header);
        }
        for chunk in self.chunks.iter() {
            for by in chunk.as_bytes() {
                to_return.push(by);
            }
        }
        to_return
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(mut value: &[u8]) -> Result<Self, Self::Error> {
        ...
        if value[0..8] == Self::STANDARD_HEADER {
            value = value.split_at(8).1;
        }
        while value.len() != 0 {
            chunks.push(Chunk::try_from(value)?);
            value = value
                .split_at(12 + chunks.last().unwrap().length() as usize)
                .1;
        }
        Ok(Self { chunks })
    }
...
mod tests {
    ...
    fn chunk_from_strings(chunk_type: &str, data: &str) -> Result<Chunk, TryFromSliceError> {
        use std::str::FromStr;
